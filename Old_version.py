...
class Network:
    def __init__(self):
        ...
        self.t = list([0] * self.V for i in range(0, self.V))
        self.c = list([0] * self.V for i in range(0, self.V))
        self.edges = list([0, 0] * self.E)
    ...
    def dijkstra_algorithm(self, graph, start):
        dist = list([sys.maxsize, -1] for i in range(self.V))
        dist[start] = [0, start]
        sptSet = [False]*self.V
    ...
    def matrix_g(self, x, x_edges):
        G = list([0]*len(x) for i in range(len(x)))
        for i in range(len(x)):
    ...
    def matrix_a(self, x, x_edges, start, finish):
        ...
        len_x = len(x)
        a = list([0]*len_x for i in range(len(v_set)))
        d = list([0]*len(v_set))
        ...
        a = np.array(a)
        ind = v_pos[finish]
        res_a = np.concatenate((a[:ind], a[ind+1:]), axis=0)
        return np.array(res_a)
    ...
    def iterations(self, x, x_edges, A):
        ...
        while True:
            ...
            AGAtr_inv = np.linalg.inv(np.dot(np.dot(A, G), A_transposed))
            tmp = I - np.dot(np.dot(np.dot(A_transposed, AGAtr_inv), A), G)
            print("agatr ", AGAtr_inv)
            print("atr*inv ", np.dot(A_transposed, AGAtr_inv))
            print("atr*inv*a ", np.dot(np.dot(A_transposed, AGAtr_inv), A))
            print("*G ", np.dot(np.dot(np.dot(A_transposed, AGAtr_inv), A), G))
            print("tmp = ", tmp)
            print("G*tmp*g = ", np.dot(np.dot(G, tmp), g))
            new_x = x - np.dot(np.dot(G, tmp), g)
            print("new x = ", new_x)
            print("norm = ", np.linalg.norm(new_x-x, ord=1))
            if np.linalg.norm(new_x-x, ord=1) < 10**-3:
                return new_x
            x = new_x
